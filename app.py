...
# ═══════════════════════════════════════════════════════════
def check_password():
    """Показывает форму ввода пароля, пока он не будет введён верно"""

    def password_entered():
        # Пароль берётся из Secrets в Streamlit Cloud (см. инструкцию)
        correct_password = st.secrets.get("APP_PASSWORD", "changeme123")
    ...
    st.markdown("## 🔒 Вход в Flashcard Generator")
    st.text_input(
        "Введите пароль",
        type="password",
